...
as `< {threshold}` rather than as a decimal. The default is `0.001`. It is
set via `clerk_options()` rather than per-table:

```{r threshold}
# Show < 0.01 instead of < 0.001
clerk_options(p_threshold = 0.01)
tbl_descriptive(clerk_example, group = sex, output = "gt") |>
...
clerk_options(
  ...
  p_threshold     = 0.001,    # below this → "< 0.001" (session-level only)
